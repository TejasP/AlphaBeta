...
class UtilityController extends AppNoAuthController {
	...
	public function cleanDuplicateElements($tableName) {
		...
		if (! $db->isConnected ()) {
			...
			/*
			 * $myData = $db->query($sql_query_1); $response = $db->rawQuery($sql_query); var_dump ($myData);
			 */
	...
	public function createandpopulateDB() {
		...
		if (! $db->isConnected ()) {
			...
			for($j = 4001; $j <= 4044; $j ++) {
				...
				/*
				 * $sql_query_in = "INSERT INTO `providers_new` (`id`,`url`, `provider_type`, `provider_name`, `website`, `mobile`, `phone`, `contact_person`, `area`, `address`, `city`, `pin_code`, `landmark`, `working_hours`, `listed_categories`) VALUES ".$j .",'".Sanitize::escape($myData[$j]['providers']['url'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['provider_type'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['provider_name'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['website'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['mobile'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['phone'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['contact_person'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['area'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['address'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['city'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['pin_code'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['landmark'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['working_hours'],'default')."'," ."'".Sanitize::escape($myData[$j]['providers']['listed_categories'],'default')."';"; $myData_in = $db->query($sql_query_in); echo $sql_query_in."</BR>";
				 */
	...
	public function updateDBwithGoogleData() {
		...
		if (! $db->isConnected ()) {
			...
			for($j = 0; $j <= 500; $j ++) {
				/*
				 * $data = array( 'Providers_new' => array( 'url' => $myData[$j]['providers']['url'], 'provider_type' => $myData[$j]['providers']['provider_type'], 'provider_name'=>$myData[$j]['providers']['provider_name'], 'website'=>$myData[$j]['providers']['website'], 'mobile'=>$myData[$j]['providers']['mobile'], 'phone'=>$myData[$j]['providers']['phone'], 'contact_person'=>$myData[$j]['providers']['contact_person'], 'area'=>$myData[$j]['providers']['area'], 'address'=>$myData[$j]['providers']['address'], 'city'=>$myData[$j]['providers']['city'], 'pin_code'=>$myData[$j]['providers']['pin_code'], 'landmark'=>$myData[$j]['providers']['landmark'], 'working_hours'=>$myData[$j]['providers']['working_hours'], 'listed_categories'=>$myData[$j]['providers']['listed_categories']) );
				 */
				...
				if ($xml ['status'] === 'OK') {
					if (count ( $xml ['result'] ['name'] ) == 1) {
						...
						if ($this->Providers_new->updateAll ( array (
								'Providers_new.google_address_1' => "'" . $xml ['result'] ['formatted_address'] . "'",
								'Providers_new.google_address_1_longi' => $xml ['result'] ['geometry'] ['location'] ['lng'],
								'Providers_new.google_address_1_lati' => $xml ['result'] ['geometry'] ['location'] ['lat'] 
						), array (
								'Providers_new.id' => $j 
						) )) {
							echo "Updated .";
						...
						if ($this->Providers_new->updateAll ( array (
								'Providers_new.google_address_1' => "'" . $xml ['result'] [0] ['formatted_address'] . "'",
								'Providers_new.google_address_1_longi' => $xml ['result'] [0] ['geometry'] ['location'] ['lng'],
								'Providers_new.google_address_1_lati' => $xml ['result'] [0] ['geometry'] ['location'] ['lat'],
								'Providers_new.google_address_2' => "'" . $xml ['result'] [1] ['formatted_address'] . "'",
								'Providers_new.google_address_2_longi' => $xml ['result'] [1] ['geometry'] ['location'] ['lng'],
								'Providers_new.google_address_2_lati' => $xml ['result'] [1] ['geometry'] ['location'] ['lat'] 
						), array (
								'Providers_new.id' => $j 
						) )) {
							echo "Updated .";
			...
			$this->autoRender = false;
			/*
			 * $this->response->type('json'); $this->response->body($xml);
			 */
		}
	}
			...
			for($j = 601; $j <= 644; $j ++) {
				...
				if ($xml ['status'] === 'OK') {
					...
					if (isset ( $xml ['result'] ['geometry'] )) {
						if (isset ( $xml ['result'] ['geometry'] ['bounds'] )) {
						...
						$data = array (
								'location' => array (
										...
										'pincode' => intval($myData [$j] ['providers_datas'] ['pin_code']),
					...
					} else if (isset ( $xml ['result'] [0] ['geometry'] )) {
						if (isset ( $xml ['result'] [0] ['geometry'] ['bounds'] )) {
						...
						if (isset ( $xml ['result'] [0] ['geometry'] ['location'] )) {
